...
3. **Factorization.** Using multiplicativity of $\mathrm{BC}$ and Fubini,
    ...
    Together these give equality, $\alpha^\star(P\otimes R)=\alpha^\star(P)\alpha^\star(R)$. The additivity of $K$ follows from the log law (see A.9): $K=-\log_2\alpha^\star$ gives
...
$\mathrm{BC}$ multiplicativity (Lemma [A.2.2](#lemma-a22-bhattacharyya-properties)); FI product structure (Prop. [A.1.8](#proposition-a18-product-structure)); log law (see A.9).