...
3. **Force weakest-link aggregation (A0–A4 + see A.6).**
Label invariance and continuity constrain any aggregator on $[0,1]^\mathcal C$ to be unanimity-respecting and monotone; the weakest-link cap follows from A3 (each coordinate upper-bounds any admissible aggregate). By Theorem A.6, the only such aggregator is the **minimum**. Hence the inner aggregator is **$\min_{c}$**.
...
- Kernel uniqueness $\Rightarrow F=\mathrm{BC}$: (see A.8).
- Log law (pinning $h$ to $-\log$): (see A.9).
- Additivity on products: (see A.10).