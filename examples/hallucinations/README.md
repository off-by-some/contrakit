...
But that precision is the difference between "AI is useless" and getting serious work done. Prompt engineering techniques can shift performance substantially without changing the model at all.
...
**Practical takeaways before we go deeper:**
- Minimize contradiction: ask for missing context, specify reference frames explicitly
- Add explicit abstention channels (let the model say "I don't know")
- Don't interpret output confidence as epistemic certainty
- Recognize that some questions are structurally unanswerable without additional information
...
You can work this out on paper before training anything. If you commit fully to Rule A, you'll be perfect on Rule A test sets—0% error. But on Rule B test sets you'll fail on all seven contradictory digits. That's 70% error, and that's your worst case under a minimax risk functional. The same logic applies in reverse if you commit to Rule B. Either way, **70% is the ceiling on worst-context performance**. No algorithm can beat it—not a neural network, not hand-coded rules, not a human expert making educated guesses.
...
**Important clarification on risk functionals:** This 70% figure is a **worst-context (minimax) error rate**—the maximum error across the two contexts. This is different from average error across both contexts (Bayes risk), which would be lower. The contradiction measure K(P) that we'll define shortly relates to average-case bounds, not worst-case bounds, but both are determined by the task geometry. The learning algorithm is working fine; the task geometry determines what's possible.
...
For the digit task above, $K = 0.35$ bits. The formula connecting $K$ to minimum **average error** (not worst-case) is:

$\text{error}_{\text{avg}} \geq 1 - 2^{-K}$

With $K = 0.35$ bits, that gives minimum average error $\geq 21.5\%$. The 70% worst-case we observed is higher because "worst case across two contexts" (minimax risk) is a stronger constraint than average error (Bayes risk), but both are bounded by the contradiction in the task structure.
...
Standard closed-set classification trained with cross-entropy strongly incentivizes confident assignment among fixed labels. Unless the task includes an explicit abstention option (and it's adequately weighted in training), the learned function will interpolate everywhere. The architecture can't express "I don't have enough information" or "this question doesn't make sense." Every input gets the same treatment: embed, transform through layers, project to output space, softmax, select highest probability. Even when the input has nothing to do with training data, this process generates a confident prediction.

The limitation isn't softmax itself—you *can* add an abstention token and calibrate confidence. The problem is the combination of closed-set assumptions, training objectives, and deployment policies that together create strong pressure toward forced commitment.
...
**Key finding:** You need witness capacity $r$ to substantially exceed $K$, often by a significant margin, before meaningful error reduction begins. The transition is gradual rather than sharp:

- When $r$ is well below $K$: Models fail consistently (error rates near 100% across all seeds)
- When $r$ substantially exceeds $K$: Models show error reduction, though practical optimization and finite capacity limits mean they rarely reach theoretical minima
- The transition occurs gradually, not at a precise $r = K$ threshold
...
A task with $K = 0.5$ bits shows high error rates until $r = 2.0$ bits—four times the contradiction level. Tasks with higher $K$ (0.79–1.16 bits) require even more witness capacity before improvement begins. This reflects practical limitations in finite networks and stochastic optimization.
...
```
====================================================================================================
SUMMARY: Gradual Phase Transition Requiring r >> K
====================================================================================================
K (bits)   r (bits)   Error Rate      r ≥ K?     Phase           Notes
----------------------------------------------------------------------------------------------------
0.5000     0.00       0.8000±0.026  ✗ No       Failure (E≈1)   Insufficient capacity
0.5000     0.50       0.8000±0.026  ✓ Yes      Failure (E≈1)   At threshold, still failing
0.5000     1.00       0.8000±0.026  ✓ Yes      Failure (E≈1)   2x capacity, still failing
0.5000     1.50       0.8000±0.026  ✓ Yes      Failure (E≈1)   3x capacity, still failing
0.5000     2.00       0.5340±0.055  ✓ Yes      Improving       4x capacity, finally improving
0.7925     0.00       0.9240±0.014  ✗ No       Failure (E≈1)   Insufficient capacity
0.7925     0.50       0.9240±0.014  ✗ No       Failure (E≈1)   Below threshold
0.7925     1.00       0.9240±0.014  ✓ Yes      Failure (E≈1)   At threshold, still failing
0.7925     1.50       0.9240±0.014  ✓ Yes      Failure (E≈1)   2x capacity, still failing
0.7925     2.00       0.8620±0.028  ✓ Yes      Marginal        3x capacity, slight improvement
1.0000     0.00       0.9700±0.021  ✗ No       Failure (E≈1)   Insufficient capacity
1.0000     0.50       0.9700±0.021  ✗ No       Failure (E≈1)   Well below threshold
1.0000     1.00       0.9700±0.021  ✓ Yes      Failure (E≈1)   At threshold, still failing
1.0000     1.50       0.9700±0.021  ✓ Yes      Failure (E≈1)   1.5x capacity, still failing
1.0000     2.00       0.9700±0.021  ✓ Yes      Failure (E≈1)   2x capacity, still failing
1.1610     0.00       0.9840±0.010  ✗ No       Failure (E≈1)   Insufficient capacity
1.1610     0.50       0.9840±0.010  ✗ No       Failure (E≈1)   Well below threshold
1.1610     1.00       0.9840±0.010  ✗ No       Failure (E≈1)   Still below threshold
1.1610     1.50       0.9840±0.010  ✓ Yes      Failure (E≈1)   Above threshold, still failing
1.1610     2.00       0.9840±0.010  ✓ Yes      Failure (E≈1)   2x capacity, still failing
====================================================================================================
Note: Transition requires r substantially exceeding K, not just r ≥ K. Even at 2-4x the 
contradiction level, optimization and finite capacity limit practical performance.
====================================================================================================
```
...
I tested this by varying the ratio of defined to undefined examples from 10% to 90% defined, holding $K = 0.5000$ bits constant. That theoretical minimum of 29.3% average error never changed—the task structure stayed identical. But observed hallucination rates ranged from 51.9% (±7.3%) at 10% defined to 98.3% (±2.1%) at 70% defined.

**This result seems counterintuitive.** Usually more training data improves performance. Here it makes things worse.

**Experimental details:**
- Dataset size held constant at 500 total examples
- Abstention represented as a separate output class with token "⊥"
- Loss function: standard cross-entropy over all classes including abstention
- Architecture: 3-layer MLP with ReLU activations
- Decision rule: argmax over output logits (no threshold)
- Hallucination computed as: (incorrect classifications on undefined inputs) / (total undefined inputs)
...
Some tasks demand incompatible behaviors across different contexts. When $K = 0.5000$ bits, at least 29% average error is guaranteed when models must commit to answers. This bound applies equally to neural networks, decision trees, hand-coded rules, or humans guessing. The impossibility is mathematical—it's baked into what you're asking for, not a quirk of any particular learning system.

**Second: Architectural pressure (insufficient $r$)**  
Closed-set training with standard objectives creates strong pressure toward confident predictions, whether prediction makes sense or not. When witness capacity $r$ is insufficient relative to task contradiction $K$—and "insufficient" typically means $r$ must substantially exceed $K$, not just match it—models cannot adequately represent epistemic uncertainty. The 75-point gap (1% with abstention, 76% forced) on $K = 0.70$ bit tasks isolates this effect cleanly.
...
**The practical insight:** minimize K where you can (clearer prompts, better task structure, providing necessary context) and ensure adequate witness capacity where you can't (explicit uncertainty mechanisms, appropriate tools, abstention options). That's how you get into the regime where this actually works.

---

## **9. Cross-Domain Applications**

If the framework only worked for neural networks, it would be interesting but limited. But the same mathematical structure appears across disparate domains. What follows is suggestive—promising empirical matches that warrant deeper investigation.
...
I computed the contradiction measure $K(P)$ for quantum measurement contexts in Bell-type scenarios. For CHSH at maximum violation (Tsirelson bound), the computation gives $K(P) \approx 0.0124$ bits per measurement pair when contexts correspond to Alice and Bob's incompatible measurement choices.

![CHSH inequality analysis showing quantum-classical boundaries](/figures/bell_chsh_analysis.png)

**Important note on quantum calculations:** The specific numerical value depends critically on how you map physical measurement contexts to the framework's "lens" objects and how you normalize the symbol space. The 0.0124 figure represents per-measurement-pair contradiction under a specific encoding. Different normalizations or aggregation schemes can yield different values in the range 0.01–0.1 bits.
...
- Probabilistic violations (Bell, KCBS): range 0.01–0.1 bits depending on normalization
- Algebraic impossibilities (Magic Square): approximately 0.13 bits

![KCBS contextuality analysis showing noise robustness](/figures/kcbs_contextuality_analysis.png)

**What this means:** The framework reproduces known quantum-classical boundaries. Whether it *explains* why quantum mechanics achieves these specific values, or merely recapitulates them through a different formalism, requires deeper investigation by physicists familiar with quantum foundations. The match is intriguing but not yet explanatory.
...
**The same mathematics we used to predict neural network hallucination appears in quantum contextuality and distributed consensus. Whether this reflects deep unity or coincidental structural similarity requires further investigation.**
...
### **Why ensembling has fundamental limits**

A common response to unreliable models is to train multiple models and average their predictions. This works for reducing variance, but **it has fundamental limits when structural contradiction is present**.

Each model in an ensemble faces the same task structure with contradiction $K$. If each model lacks sufficient witness capacity or training to handle this contradiction, averaging their outputs won't eliminate the structural floor—you're averaging guesses on cases where none of the models have adequate information.

More precisely: if we measure performance in terms of **expected log loss** (the natural unit for information-theoretic bounds), each model incurs a penalty of at least $K$ bits when forced to reconcile contradictory contexts. The expected loss of an ensemble $E_{\text{ensemble}}$ satisfies a similar bound because the structural contradiction hasn't been resolved—you've simply averaged over multiple attempts to approximate an impossible unified distribution.

For **0-1 classification error**, the relationship is more complex, but the principle holds: ensembling cannot break through floors imposed by task geometry. The average of wrong guesses is still systematically wrong when the wrongness is structural rather than stochastic.

**The practical implication:** Ensembling helps with variance reduction and model-specific biases, but it cannot compensate for inadequate witness capacity or structural contradictions in the task itself.

### **Why PAC learning theory needs careful interpretation**

PAC (Probably Approximately Correct) learning theory provides the mathematical foundation for most machine learning. It guarantees that with enough training data, you can get arbitrarily close to the optimal error rate for your hypothesis class.

**When $K > 0$, we're in the agnostic learning regime rather than the realizable case.** There is no hypothesis that achieves perfect agreement across all contexts—the structural contradiction makes it impossible. The minimum error floor is not statistical (arising from finite data) but structural (arising from the task itself).

Our experiments confirm this: for $K = 0.5$ bits (minimum average error 29.3%), no training configuration broke through this floor, regardless of data quantity or composition. More data doesn't help because there's no better solution to find within the hypothesis class.

**PAC learning doesn't "break"—it moves from realizable to agnostic assumptions. The floor becomes irreducible within the given hypothesis class.**

### **Why errors compound in reasoning chains**

When you chain multiple reasoning steps together, each with unresolved contradictions, errors propagate multiplicatively, not additively. If you have $n$ sequential tasks, each with contradiction $K_i$ and your model has witness capacity $r$, the probability of getting through the entire chain without error degrades exponentially with chain length.
...
**Status of the framework across domains:**

For neural networks: The results are solid. $K(P)$ predicts hallucination floors from task structure before training. The gradual phase transition requiring $r \gg K$ holds across 100+ runs with different architectures and random seeds. Witness capacity trained on structural contradictions generalizes to epistemic uncertainty on out-of-distribution inputs. This is reproducible, tested, confirmed.

For quantum systems: The results are suggestive. The computed values match known quantum-classical boundaries, but whether $K(P)$ explains *why* these values emerge from quantum mechanics, or just happens to reproduce them, needs physicists examining the framework carefully. The critical test would be: take a quantum channel with experimentally verified capacity (superdense coding works well), model Alice and Bob's incompatible measurement choices as contradictory decoders, compute $K(P)$ from their measurement contexts, and check if $C_{\text{classical}} - K(P)$ matches the verified quantum capacity. If it does, $K(P)$ captures the quantum-classical gap through pure information geometry. If it doesn't, the framework applies to designed systems but not fundamental physics.
...
# Output: K ≈ 0.500 bits (predicts minimum average error ≥ 29.3%)
```
...
# Set quantum correlation probabilities (at Tsirelson bound)
# Note: specific values depend on measurement angle encoding
with alice_0_bob_45:
...
# Output: K ≈ 0.0124 bits (per measurement pair, specific encoding)
# Note: Values vary with normalization choices
```
...
print(f"Predicted minimum hallucination (avg): ≥{1 - 2**(-task_behavior.K):.1%}")
# Output: K = 0.500 bits, minimum average error ≥ 29.3%
```
...
- The witness capacity threshold phenomenon is reproducible across 100+ training runs, though transitions are gradual and require $r \gg K$ rather than just $r \geq K$
- Witness capacity trained on structural contradictions generalizes to epistemic uncertainty on out-of-distribution data
- The mathematical framework has unique properties forced by basic axioms (Theorems 1-5)
- Training composition affects distance from theoretical floor but cannot eliminate it

**Promising but needs independent replication:**
- $K(P)$ computations for quantum scenarios match known boundaries (CHSH, Magic Square), but the mapping from physical contexts to framework objects needs scrutiny
- Byzantine consensus applications show adaptive message savings in tested scenarios; larger scale validation needed
- Compression and channel capacity theorems (6-9) proven for the formalism but need testing on realistic communication scenarios

**Speculative (directions for future research):**
- Whether the framework captures fundamental physics (quantum mechanics) or only designed systems (neural networks, protocols)
- Connections to philosophical questions about epistemology and the nature of information
- Whether these principles apply broadly to intelligence or are specific to current architectural choices
- The precise relationship between $K(P)$ and other measures of inconsistency or contextuality in quantum foundations

**Critical open questions:**

The quantum numbers particularly need scrutiny. The framework gives values that match experimental quantum-classical boundaries, but "matching the numbers" is different from "explaining why these must be the numbers." The test case that would settle it: take a quantum channel with experimentally verified capacity, model measurement incompatibilities as contradictory decoders, compute $K(P)$, and check if $C_{\text{classical}} - K(P)$ matches verified quantum capacity.

For neural networks, the digit experiment is the flagship evidence because everything is controlled and transparent. The witness capacity experiments show the phenomenon but with more moving parts. The training composition results are surprising and need replication with different architectures and task types.
...
Some immediate practical changes:

**Task design:**
- Provide all context needed to resolve potential ambiguities
- Make reference frames explicit (timezone, units, perspective)
- Recognize when a question is inherently underspecified
- Don't force commitment when abstention is the correct response

**Architecture:**
- Add explicit abstention mechanisms ("I don't know" tokens)
- Design loss functions that reward appropriate uncertainty
- Calibrate confidence to reflect epistemic state, not just prediction strength
- Ensure training includes adequate undefined/out-of-scope examples

**Deployment:**
- Monitor for distribution shift that increases effective $K$
- Build systems that degrade gracefully rather than confidently hallucinate
- Use ensembles for variance reduction, not to overcome structural floors
- Chain reasoning carefully, recognizing error compounds exponentially
...
Here's what emerges from this work: better AI alone may not be sufficient. We might need better mathematics for thinking about information itself—mathematics that can represent contradiction, paradox, and epistemic uncertainty as first-class concepts instead of edge cases.

To truly model intelligence, uncertainty, or epistemic processes, we may need to go beyond classical probability theory. Kolmogorov's axioms might be incomplete for these phenomena. Yet that's what neural networks are fundamentally built on.

This doesn't seem impossible. Witness capacity is a mechanism we've implemented and tested for getting the behaviors people want—saying "I don't know," reasoning about uncertainty, avoiding hallucination on undefined inputs. It's not just theoretical; the experiments show it working. Neural networks with sufficient witness capacity ($r \gg K$) abstain correctly on contradictory inputs, while those without it hallucinate systematically.

**The architecture exists. It's just not how things work by default currently.**

If this research direction proves fruitful, we may need to address these fundamentals before we can build systems that truly reason about their own limitations. What exists now is powerful but incomplete.

There's a historical pattern here. Scientific paradigms filter and reinterpret earlier work, and that filtering—including the misunderstandings—is part of how revolutions happen. Kuhn pointed this out. It's possible we're in one of those moments. We took brilliant ideas about information and logic, applied them everywhere without fully understanding the trade-offs, and now we're hitting limits.
...
- More rigorous testing of the quantum connections by physicists
- Larger-scale Byzantine consensus implementations
- Testing on realistic communication channels
- Independent replication of the witness capacity experiments
- Exploring whether similar principles apply to human reasoning and epistemology

But the core insight—that some failures are mathematical rather than algorithmic—changes how we think about building reliable AI systems. It suggests that progress requires not just bigger models or more data, but better understanding of the geometry of information itself.

---

*The mathematical details are in [contrakit on GitHub](https://github.com/off-by-some/contrakit) if you're curious. Run `examples/hallucinations/experiment_9` to see the gradual phase transitions in witness capacity, or `examples/quantum/CHSH.py` to reproduce the quantum contextuality calculations. The framework is open for scrutiny and independent testing.*