...
Production language models seem to show the same bottleneck we found in our experiments. They must produce responses for every input, even when "I don't know" would be more appropriate.
...
Multi-hop reasoning makes this worse. Each reasoning step accumulates contradiction. More importantly, each step compounds architectural forcing. We predict that in a $5$-step reasoning chain where each step has a $29\%$ structural floor, architectural forcing could add roughly $35\%$ per step. By step $5$, fabrication could approach $95$--$100\%$, even though the theoretical minimum per step is only $29\%$. This predicts why long chains of thought degrade—not because contradiction compounds without bound, but because forced commitment compounds without abstention support at each step.
